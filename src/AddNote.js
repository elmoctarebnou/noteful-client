...
export default class AddNote extends React.Component{
    ...
    render(){  
        const selectOptions = this.state.foldersList.map((folder)=> <option key={folder.id} id={folder.id}>{folder.name}</option>)
        return (
            <form className='addNote' onSubmit={this.handelSubmit}>
                ...
                <br/>
                <input onChange={this.updateName} type='text' placeholder='Add note name' required/>
                <br/>
...
AddNote.propTypes = {
    note: PropType.objectOf(PropType.shape({
